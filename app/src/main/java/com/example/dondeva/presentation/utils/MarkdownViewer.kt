...
@Composable
fun MarkdownViewer(text: String) {
    ...
    for (token in tokens) {
        ...
        when (token.type) {
            ...
            TokenType.HEADING -> {
                ...
                builder.addStyle(
                    if (headingLevel == 1) MaterialTheme.typography.headlineSmall.toSpanStyle()
                    else MaterialTheme.typography.titleMedium.toSpanStyle(),
                    styleStart,
                    builder.length,
                )
            }
